...
import { useCreateQuiz } from '@/hooks/assessment/useQuizzes'
import { builderQuestionToPayload } from '@/lib/quiz/helpers'
import type { CreateQuizPayload } from '@/types/assessment'
...
// ΓöÇΓöÇΓöÇ Question type definitions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ QuestionEditor (local component) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    <div className="q-item editing">
      ...
      <Field label="Question prompt">
        <textarea
          className="input"
          rows={2}
          placeholder="Enter your questionΓÇª"
          value={q.prompt}
      ...
      <div style={{ marginTop: 14 }}>
            ...
            {(q.blanks ?? []).map((b, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <input
                  ...
                  placeholder="Accepted answerΓÇª"
                  value={b}
          ...
          <div>
            <Field label="Grading rubric">
              <textarea
                ...
                placeholder="Describe what a correct answer looks likeΓÇª"
                value={q.rubric ?? ''}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {(q.pairs ?? []).map((pair, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input
                  className="input" style={{ flex: 1 }}
                  placeholder="Left sideΓÇª"
                  value={pair.l}
                  onChange={e => updatePair(i, 'l', e.target.value)}
                />
                <span style={{ color: 'var(--muted)' }}>Γçä</span>
                <input
                  className="input" style={{ flex: 1 }}
                  placeholder="Right sideΓÇª"
                  value={pair.r}
        ...
        {q.type === 'upload' && (
          ...
          >
            <div style={{ fontSize: 24, marginBottom: 6 }}>Γåæ</div>
...
export default function QuizBuilderPage() {
  ...
  const [quizType, setQuizType] = useState<'graded' | 'practice' | 'survey'>('graded')
  ...
  const [questions, setQuestions] = useState<Question[]>([makeQuestion('mcq')])
  ...
  const handlePublish = () => {
    setSubmitError(null)
    createQuiz.mutate(buildPayload(true), {
      onSuccess: () => router.push('/teacher/quizzes'),
      onError: () => setSubmitError('Could not publish quiz. Check your questions and try again.'),
    })
  }

  const handleSaveDraft = () => {
    setSubmitError(null)
    createQuiz.mutate(buildPayload(false), {
      onSuccess: () => router.push('/teacher/quizzes'),
      onError: () => setSubmitError('Could not save draft.'),
    })
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 24, alignItems: 'start' }}>
      ...
      <div>
        ...
        <div className="page-head">
          ...
          <div className="row" style={{ gap: 8 }}>
            <Button variant="secondary" size="sm" onClick={handleSaveDraft} disabled={createQuiz.isPending || !activeCourseId}>
              Save draft
            </Button>
            <Button variant="brand" size="sm" icon={Check} onClick={handlePublish} disabled={createQuiz.isPending || !activeCourseId}>
              {createQuiz.isPending ? 'Publishing…' : 'Publish'}
            </Button>
          </div>
        </div>
        ...
        <div className="card card-pad" style={{ marginBottom: 24, display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <Field label="Course">
            <select className="select" value={activeCourseId} onChange={e => setCourseId(e.target.value)} disabled={coursesLoading}>
              {courses.map(c => (
      ...
      <div style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="card card-pad">
          ...
          {/* Progress bar showing grading progress (mock) */}
